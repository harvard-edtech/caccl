...
describeS('Server > Initialization', function (driver) {
  itS('Throws error when installation credentials are excluded and LTI is enabled', async function () {
  ...
  itS('Throws error when developer credentials are excluded and auth is enabled', async function () {
    await testConfiguration(driver, {
...
describeS('Server > Express App Configuration', function () {
  itS('Uses custom express app', async function (driver) {
    await testConfiguration(driver);
  });
});
